...
| Библиотека в две колонки, карточка показывает ближайшее напоминание | Шифрование базы |
| Корзина: вкладка с часом на возврат, потом заметка стирается сама | |
...
| В уведомлении две кнопки: «Готово» и «Отложить» | |
| «Отложить» на выбранный срок (по умолчанию час), в настройках | |
| Экран «Напоминания»: заметки, у которых что-то впереди, ближайшее сверху | |
...
| Быстрая заметка голосом: распознавание на устройстве, разбор времени, напоминание заранее | Разбор дат («12 марта») и повторов («каждый день») в голосе |
| Теги: фильтр в библиотеке и строка под карточкой | |
| Четыре тёмные палитры и 8 языков интерфейса — всё в настройках | Смена иконки приложения (отложена, см. docs/plans) |

Если чего-то нет в левой колонке, значит этого не написали — сколько бы места
под это ни было заложено в схеме базы.

### 0.1 Быстрая заметка голосом

Кнопка с микрофоном в библиотеке — и ярлык «Продиктовать» по долгому нажатию на
иконку приложения, который открывает микрофон сразу. Сказали «встреча, 15:00» —
появилась заметка «Встреча» и напоминание в 14:30. Запас в полчаса, время для
фраз без времени и день, на который они попадают («сегодня», «завтра» или через
столько-то дней), меняются в **Настройки → Быстрые заметки**.

Что понимает разбор фразы (`src-tauri/src/domain/quick_notes/phrase.rs`):

| Сказано | Что получится |
| --- | --- |
| `встреча. 15:00`, `в 15 30`, `в 15 часов 30 минут` | сегодня в 15:30, а если время прошло — завтра |
| `в три часа`, `в девять`, `полвторого`, `в половине третьего`, `без пятнадцати шесть`, `полдень` | числа словами; час читается как дневной |
| `в 9 утра`, `в семь вечера`, `в час дня` | часть суток задаёт половину суток явно |
| `завтра`, `послезавтра`, `в пятницу` | ближайший такой день |
| `завтра утром`, `вечером` | 09:00 / 13:00 / 19:00 / 22:00 |
| `через 20 минут`, `через час` | таймер: отсчёт от «сейчас», запас не вычитается |
| `купить молоко` | напоминание на время из настроек (по умолчанию 19:00) |

**Цифры и слова разбираются по-разному, намеренно.** `15:00` называет час из
двадцати четырёх и берётся как есть. «В три» — нет: на циферблате нет «после
полудня», поэтому сказанный словом час читается как дневной, а если тот уже
прошёл — как его двойник через двенадцать часов. «В девять» в полдень — это
девять вечера; «в три» в четыре часа дня — три завтра.

Даты («12 марта») и повторы («каждый день») не разбираются: непонятое слово
остаётся в заголовке, а не превращается в неправильное время молча. По тому же
правилу числа, которые временем не являются, остаются на месте: «на 20 минут»,
«в 2 раза больше», «встреча 12.03», «матч 14-15» — заметка сохраняет число и
получает напоминание из настроек.

Распознавание — только на устройстве: плагин просит у Android офлайн-модель
(`createOnDeviceSpeechRecognizer`, `EXTRA_PREFER_OFFLINE`). Язык подбирается по
тому, что телефон реально умеет (`checkRecognitionSupport`, Android 13+):
интерфейс → язык телефона → русский. Для татарского и башкирского
распознавания не существует нигде, поэтому приложение на этих языках слушает
по-русски и пишет об этом на экране. Если модель не скачана — предлагает
скачать её одной кнопкой. На Android 11 и ниже офлайн — только пожелание
системе, и экран честно об этом предупреждает. Звук никуда не пишется: в
WebView приходят громкость для визуализации и текст.

Если напоминание поставить не удалось — выключены уведомления, названное время
уже прошло — заметка всё равно создаётся, а причина показывается в плашке
вместе с кнопкой, которая ведёт туда, где это чинится. В самой плашке: что
сказали, на когда поставлено, «Открыть», «Отменить» (заметка уходит в корзину)
и сдвиги времени ±15 минут / ±1 час — потому что две ошибки, которые у речи
действительно бывают, это неверные слова и неверный час.
...
Пароли и пути этой машины живут в одном файле — `.env` в корне репозитория,
он в `.gitignore`. Образец с пустыми значениями — `.env.example`:
...
```dotenv
ANDROID_KEYSTORE_FILE=/Users/ВАШ_ПОЛЬЗОВАТЕЛЬ/.android-keystores/ximakeeps-proxima812-20260802.jks
ANDROID_KEYSTORE_PASSWORD=ВАШ_ПАРОЛЬ
ANDROID_KEY_ALIAS=ximakeeps
ANDROID_KEY_PASSWORD=ВАШ_ПАРОЛЬ
```

Там же можно задать `JAVA_HOME`, `ANDROID_HOME` и `ANDROID_NDK_VERSION`, если
инструменты стоят не там, где их ищет `scripts/android-env.sh`.

Gradle читает подпись не из `.env`, а из `keystore.properties` — его собирает
`scripts/android-env.sh` при каждом запуске:

```bash
source ./scripts/android-env.sh   # пишет src-tauri/gen/android/keystore.properties
```

Файл перезаписывается всегда, а не только когда его нет: сменённый в `.env`
пароль и залежавшийся рядом старый файл — ровно та беда, ради которой этот
крюк и сделан.

Конфигурация подписи активна только когда `keystore.properties` существует;
иначе release-сборка останавливается с понятным сообщением вместо выпуска
неподписанного APK.
